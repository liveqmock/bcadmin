  ...
  <div class="charge-add">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb>编辑</el-breadcrumb>
      </el-breadcrumb>
    </div>
    <div class="form-data">
        ...
        <el-form-item label="佣金累加：">
          ...
          <div class="tabPanel">
            <el-row v-show="formData.formulartype === 'course_amount'">
              ...
              <el-col :span="4">
                <el-input v-model.number="ratioDefault1"></el-input>
              </el-col>
            ...
            <el-row v-show="formData.formulartype === 'sale_amount'">
              ...
              <el-col :span="4">
                <el-input v-model.number="ratioDefault2"></el-input>
              </el-col>
              <el-col :span="1" style="text-align: center">%</el-col>
              <el-col :span="2">
            ...
            <el-row v-for="(c, i) in employeeFormularDetails1" :key="i" v-show="formData.formulartype === 'course_amount'">
              ...
              <el-col :span="1" style="text-align: center">{{ formData.formulartype === 'course_amount' ? '课时' : '金额' }},</el-col>
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        getChargeDetail () {
          let that = this
          axios.get(URL.api_name + 'backofficeapi/employee/formular/find.do', {
            params: {
            ...
            if (res.data.status === 'success') {
              ...
              that.employeeFormularDetails1 = res.data.data.employeeFormularDetails
              if (this.formData.formulartype === 'course_amount') {
                that.ratioDefault1 = res.data.data.ratioDefault
                that.employeeFormularDetails1 = res.data.data.employeeFormularDetails
              } else if (this.formData.formulartype === 'sale_amount') {
                that.ratioDefault2 = res.data.data.ratioDefault
                that.employeeFormularDetails2 = res.data.data.employeeFormularDetails
              }
            }
          })
        },
        addCharge (formName) {
          ...
          if (this.formData.formulartype === 'course_amount') {
            if (!reg.test(this.ratioDefault1)) {
              this.$message({
                type: 'error',
            ...
            for (let c of this.employeeFormularDetails1) {
              if (c.min && !courseReg.test(c.min)) {
                ...
                return
              } if (c.percent && !reg.test(c.percent)) {
                this.$message({
                  type: 'error',
            ...
            if (!reg.test(this.ratioDefault2)) {
              this.$message({
                type: 'error',
          ...
          this.$refs[formName].validate((valid) => {
            if (valid) {
              that.isLoading = true
              if (this.formData.formulartype === 'course_amount') {
                this.formData.ratioDefault = this.ratioDefault1
                this.formData.employeeFormularDetails = this.employeeFormularDetails1
              } else if (this.formData.formulartype === 'sale_amount') {
                this.formData.ratioDefault = this.ratioDefault2
                this.formData.employeeFormularDetails = this.employeeFormularDetails2
              }
              axios.post(URL.api_name + 'backofficeapi/employee/formular/update.do', that.formData).then((res) => {
                if (res.data.status === 'success') {
                  that.$message({
