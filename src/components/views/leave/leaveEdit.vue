    ...
    <div class="form-data">
      <el-form label-width="100px">
        ...
        <el-form-item label="意见:">
          <el-col :span="12">
            <el-input type="textarea" :rows="4" v-model="approveRemark"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="onSubmit('audit_pass')" :loading="loading">
            {{ loading ? '正在提交' : '同意' }}
          </el-button>
          <el-button type="primary" @click="onSubmit1('audit_reject')" :loading="loading1">
            {{ loading1 ? '正在提交' : '驳回' }}
          </el-button>
...
<script>
    ...
    export default {
      ...
      methods: {
        onSubmit (status) {
          var that = this
          if (that.approveRemark.length > 150) {
            that.$message({
              type: 'error',
              message: '审批意见最多输入150个字符'
            })
            return
          }
          that.loading = true
