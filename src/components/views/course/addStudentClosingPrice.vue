<template>
  <div class="choose-way">
    ...
    <div class="form-data">
      <el-form label-width="100px" v-if="formData">
        <el-form-item class="no-margin" label="订单总金额：">
          ¥{{ parseFloat(formData.order.price).toFixed(2) }}
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item class="no-margin" label="应付金额：">
          ¥{{ parseFloat(formData.order.paid).toFixed(2) }}
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      confirmPay () {
        ...
        that.loading = true
        axios.post(URL.api_name + 'backofficeapi/course/order/join/closingPrice.do', {
          orderId: this.formData.order.id,
          ...
          if (res.data.status === 'success') {
            // 创建支付记录成功后
            that.$router.push({
              path: '/addStudentDetailF/' + that.formData.order.id
            })
          } else {
            that.loading = false
            that.$message({
