...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail (dateType) {
        var that = this
        axios.get(URL.api_name + 'backofficeapi/course/arrange/choose/two/time.do', {
          params: {
      ...
      sure () {
        ...
        that.loading = true
        axios.post(URL.api_name + 'backofficeapi/course/arrange/choose/two/confirm.do', {
          coachId: that.$route.params.coachId,
          courseId: that.$route.params.courseId,
          courseLevel: that.$route.params.courseLevel,
          memberId: that.$route.params.courseMemberId,
          courseTypeCode: that.$route.params.courseTypeCode,
          ...
          if (res.data.status === 'success') {
            that.$message({
              message: '提交成功，接下来跳转到确认订单页面',
              type: 'success',
              duration: 2000,
              onClose: function () {
                that.$router.push({
                  path: '/studentCheckSure/' + res.data.data.orderId
                })
              }
            })
          } else {
