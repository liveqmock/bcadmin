    ...
    methods: {
      authorize (obj) {
        if (obj) {
          ...
          this.loading = true
          axios.get(URL.api_name + 'backofficeapi/course/arrange/freeSingle.do', {
            params: {
      ...
      goPay () {
        let chajia = this.formData.courseTotalPrice - this.formData.principalPrice
        this.$router.push({
          path: '/studentCheckPay/' + this.$route.params.orderNo + '/' + chajia
        })
      },
      getDetail () {
        var that = this
