    ...
    <div class="form-data">
      <el-form label-width="100px">
        ...
        <el-form-item>
          ...
          <el-input :min="0" :disabled="!shopCard" style="width: 200px" v-model.number="shopPay"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    methods: {
      ...
      confirmPay () {
        ...
        axios.post(URL.api_name + 'backofficeapi/order/pay.do', this.formData).then((res) => {
          if (res.data.status === 'success') {
            // 支付成功后手动调用减库存接口
            axios.post(URL.api_name + 'merchandiseapi/stock/consume.do', this.formData).then(res => {
              if (res.data.status === 'success') {
                // 创建支付记录成功后，订单详情
                that.$router.push({
                  path: '/goodsOrderDetail/' + that.formData.order.id
                })
              }
            })
          } else {
...
//        }
    ...
    computed: {
      ...
      changeAmount () {
        return (this.payAmount - this.formData.order.paid).toFixed(2)
      }
    }
  }
</script>
