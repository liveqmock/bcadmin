  ...
  <div class="add-goodsType">
    ...
    <div class="form-data">
      <el-form label-width="100px" ref="formData" :model="formData" :rules="rules">
        ...
        <el-form-item>
          <el-button :loading="loading" type="primary" @click="addGoodsType('formData')">{{ loading ? '正在保存' : '保存' }}</el-button>
    ...
    methods: {
      ...
      addGoodsType (name) {
        ...
        that.$refs[name].validate((valid) => {
          if (valid) {
            that.loading = true
            axios.post(URL.api_name + 'merchandiseapi/product/type/create.do', this.formData).then(res => {
              if (res.data.status === 'success') {
                that.$message({
                  type: 'success',
                  message: '添加成功',
