      ...
      <el-form label-width="120px">
        ...
        <div class="list-item">
          <el-row v-for="(o, i) in order.items" :key="i">
            ...
            <el-form-item label="积分：" v-show="o.userScore">
              {{ o.userScore }}
            </el-form-item>
            <el-form-item label="总金额：" v-if="!order.integralPay">
              ¥{{ o.price * o.quantity | formatMoney }}
            </el-form-item>
            <el-form-item label="总积分：" v-if="order.integralPay">
              {{ o.userScore * o.quantity }}积分
            </el-form-item>
          </el-row>
        </div>
        <el-form-item label="订单总金额：" v-if="!order.integralPay">
          ¥{{ order.order.price }}
        </el-form-item>
        <el-form-item label="订单总积分：" v-if="order.integralPay">
          {{ totalScore }}积分
        </el-form-item>
        ...
        <el-form-item label="支付方式：">
          <el-row v-for="(p, i) in order.payments" :key="i">
            <span>{{ p.type }}：</span>
            <span v-if="!order.integralPay">¥{{ p.amount }}</span>
            <span v-if="order.integralPay">{{ p.amount }}积分</span>
          </el-row>
        </el-form-item>
      ...
      <div style="font-family: '微软雅黑'">
        ...
        <div v-for="(o, i) in order.items" :key="i">
          <div style="display: flex; font-size: 10px; color: #000; line-height: 24px;">
            <span style="padding-left: 12px; width: 150px; overflow: hidden;white-space: nowrap; text-overflow: ellipsis">{{ o.itemDefName }}</span>
            <span style="flex-grow: 1; padding-right: 12px; text-align: right">x{{ o.quantity }}</span>
          </div>
          <div v-if="!order.integralPay" style="display: flex; font-size: 10px; color: #000; line-height: 24px;">
            <span style="padding-left: 12px; ">单价</span>
            <span style="flex-grow: 1; padding-right: 12px; text-align: right">¥{{ o.price }}</span>
          </div>
          <div v-if="order.integralPay" style="display: flex; font-size: 10px; color: #000; line-height: 24px;">
            <span style="padding-left: 12px; ">积分</span>
            <span style="flex-grow: 1; padding-right: 12px; text-align: right">{{ o.userScore }}</span>
          </div>
        </div>
        <!-- 分割线 -->
        <div style="border-bottom: 1px dashed #000;"></div>
        <div v-if="!order.integralPay" style="display: flex; font-size: 10px; color: #000; padding-top: 5px; line-height: 20px;">
          <span style="padding-left: 12px; padding-right: 5px;">总计金额:</span>
          <span style="flex-grow: 1">¥{{ order.order.price }}</span>
        </div>
        <div v-if="order.integralPay" style="display: flex; font-size: 10px; color: #000; padding-top: 5px; line-height: 20px;">
          <span style="padding-left: 12px; padding-right: 5px;">总积分:</span>
          <span style="flex-grow: 1">{{ totalScore }}</span>
        </div>
        ...
        <div style="display: flex; font-size: 10px; color: #000; line-height: 32px;"
          ...
          <span v-if="!order.integralPay" style="flex-grow: 1;">¥{{ p.amount }}</span>
          <span v-if="order.integralPay" style="flex-grow: 1;">{{ p.amount }}积分</span>
        </div>
        <div style="display: flex; font-size: 10px; color: #000; padding-top: 5px; line-height: 20px;">
    ...
    computed: {
      totalScore () {
        ...
        this.order.items.map((item) => {
          if (item.userScore) {
            sum += item.userScore * item.quantity
          }
        })
        return sum
      },
