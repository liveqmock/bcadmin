  ...
  <div class="renew-detail">
      ...
      <el-form label-width="120px">
        ...
        <div class="list-item">
          <el-row v-for="(o, i) in order.items" :key="i">
            ...
            <el-form-item label="有效期：" v-if="o.validity">
              {{ o.validity }}天
            </el-form-item>
            <el-form-item label="使用日期：" v-if="o.useDate">
              {{ o.useDate }}
            </el-form-item>
        ...
        <el-form-item label="终端：">
          {{ order.order.saleSource | formatSource }}
        </el-form-item>
    ...
    <div id="order" style="display: none">
      <div style="font-family: '微软雅黑';">
        ...
        <div style="display: flex; font-size: 10px; color: #000; padding-top: 5px; line-height: 20px;" v-for="(d, i) in order.discounts" :key="i">
          <span style="padding-left: 12px; padding-right: 5px;">{{ d.type }}:</span>
          <span style="flex-grow: 1">¥{{ d.effect }}</span>
        </div>
        <div style="display: flex; font-size: 10px; color: #000; padding-top: 5px; line-height: 20px;" v-for="(d, i) in order.coupons" :key="i">
    ...
    filters: {
      formatSource (val) {
        if (val === 'android app') {
          ...
          return 'IOS APP'
        } else if (val === 'ios ipad') {
          return 'IOS PAD'
        } else if (val === 'web admin') {
          return 'PC端'
        } else if (val === 'wx') {
          return '微信公众号'
        } else if (val === 'vending machine') {
          return '售票机'
        } else {
          return ''
        }
