<template>
  <div class="list">
    ...
    <el-form :model="formData" :rules="rules" ref="formData" label-width="130px">
      ...
      <el-form-item label="活动人数：" required prop="countLimit">
        <el-col :span="9">
          <el-input type="number" v-model.number="formData.countLimit"></el-input>
        </el-col>
      </el-form-item>
      ...
      <el-form-item label="支付类型：" required>
        <el-checkbox-group v-model="signTypes" @change="checkTypes">
          <el-row>
            <el-col :span="3">
              <el-checkbox label="1">活动价格
                <el-input type="number" v-model.number="formData.signMoney"></el-input>　元
              </el-checkbox>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="3">
              <el-checkbox label="2">消耗积分
                <el-input type="number" v-model.number="formData.signPoint"></el-input>　分
              </el-checkbox>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="3">
              <el-checkbox label="3">抵消课程
      ...
      <el-form-item>
        <el-button type="primary" :loading="isLoading" @click="addActivity('formData')">
            {{ isLoading ? '正在保存' : '保存' }}
        </el-button>
...
export default {
  ...
  data () {
    var checkNumber = (rule, value, callback) => {
      let nums = /^[1-9]\d*(?!.*\.)/
      if (!nums.test(value)) {
        return callback(new Error('格式不正确，请输入正整数或0'))
      } else {
        callback()
      }
    }
    ...
    addActivity (formName) {
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          that.formData.eventPictures = []
          for (let i = 0; i < that.rinkImages.length; i++) {
            that.formData.eventPictures.push({
              pictureName: that.rinkImages[i].response.data.pictureName,
