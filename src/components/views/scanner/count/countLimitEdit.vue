...
<script>
    ...
    export default {
      ...
      methods: {
        getInfo () {
          var that = this
          axios.get(URL.api_name + 'backofficeapi/pass/limit/find-by-id.do', {
            params: {
        ...
        onSubmit (name) {
          ...
          that.$refs[name].validate((valid) => {
            if (valid) {
              ...
              that.loading = true
              axios.post(URL.api_name + 'backofficeapi/pass/limit/update.do', that.formData).then(res => {
                if (res.data.status === 'success') {
                  that.$message({
                    type: 'success',
                    message: '修改成功',
